    ...
    break;
       case 2:{
            ...
            if (aerolineas[getPositionAeroline(aerolinea_id)].getCantidadAvionesFlota() < 1){
                ...
                cin>>matricula;
                cout<<aerolineas[getPositionAeroline(aerolinea_id)].eliminarAvion(matricula)<<endl;
            }
        };
    break;
...
void Control::clientsOfCompania(){
  ...
  cout << "\n :::::::::Total clientes agencia o aerolinea:::::::" << endl;
  do {
    cout << "Agencia o aerolinea? (agencia, aerolinea)" << endl;
    cin >> compania;
  } while(compania != "agencia" && compania != "aerolinea");
  ...
  if(compania == "agencia"){
    ...
    cout << "Total de clientes: "+to_string( this->agencias[getPositionAgency(agencia_id)].getClients().size() ) << endl;
  }
...
void Control::getTotalVentas(){
    ...
    if(compania == "agencia"){
      ...
      if(idTransaciones.size() > 0){
        for (int i = 0; i < idTransaciones.size(); i++) {
          this->tiquetes[getPositionTicket(idTransaciones[i])];
          total = total + this->tiquetes[getPositionTicket(idTransaciones[i])].getPrecio();
        }
      }
    }
